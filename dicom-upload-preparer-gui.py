...
class DicomUploadPreparerGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(root, text=APP_TITLE, font=("TkDefaultFont", 18, "bold")).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 4))
        ttk.Label(root, text="Prepare DICOM studies for review, transfer, or upload.").grid(row=1, column=0, columnspan=3, sticky="w", pady=(0, 18))
        ttk.Label(root, text="DICOM source folder").grid(row=2, column=0, sticky="w", pady=6)
        ttk.Entry(root, textvariable=self.source_var).grid(row=2, column=1, sticky="ew", padx=(12, 8), pady=6)
        ...
        ttk.Entry(root, textvariable=self.output_var).grid(row=3, column=1, sticky="ew", padx=(12, 8), pady=6)
        ...
        ttk.Checkbutton(options, text="Anonymize DICOM files before writing them", variable=self.anonymize_var).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 8))
        ttk.Checkbutton(options, text="Create ZIP archives", variable=self.create_zip_var, command=self._update_controls).grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(root, text="Privacy: DICOM files can contain personal medical information. Anonymization is enabled by default in this graphical interface.", wraplength=760).grid(row=5, column=0, columnspan=3, sticky="w", pady=(0, 12))
        actions = ttk.Frame(root)
        actions.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        actions.columnconfigure(0, weight=1)
        self.prepare_button = ttk.Button(actions, text="Prepare DICOM files", command=self._start_processing)
        ...
        ttk.Label(root, textvariable=self.status_var, anchor="w").grid(row=9, column=0, columnspan=3, sticky="ew", pady=(8, 0))
    ...
    def _validate(self):
        ...
        if not source.is_dir():
            ...
            return None
        output = Path(output_text).expanduser().resolve() if output_text else source.parent / "dicom-prepared"
        ...
        try:
            ...
            messagebox.showerror(APP_TITLE, "Choose an output folder outside the DICOM source folder.\n\nThis prevents generated files from being scanned again on a later run.")
            return None
        try:
    ...
    def _engine_path(self):
        names = ["prepare-dicom-upload.exe", "prepare-dicom-upload"] if os.name == "nt" else ["prepare-dicom-upload"]
        bases = []
        if getattr(sys, "frozen", False):
            bases.append(Path(sys.executable).resolve().parent)
        bases.extend([Path(__file__).resolve().parent, Path.cwd()])
        for base in bases:
            for name in names:
                candidate = base / name
    ...
    def _start_processing(self):
        ...
        if engine is None:
            messagebox.showerror(APP_TITLE, "Cannot find the DICOM processing engine.\n\nIf you downloaded a desktop release, keep all supplied files together. If you downloaded the source code, keep dicom-upload-preparer-gui.py next to prepare-dicom-upload.")
            return
        ...
        command += [str(source), "-o", str(output), "--max-zip-mb", str(max_zip)]
        ...
        threading.Thread(target=self._run_process, args=(command, output), daemon=True).start()
    ...
    def _run_process(self, command, output):
        try:
            ...
            self._process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", bufsize=1, creationflags=creationflags)
...
def main():
    DicomUploadPreparerGUI().mainloop()
